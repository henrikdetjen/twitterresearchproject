...
public class ResultPoolingImpl implements IResultPooling {
	...
	public ClusterElement getClusters( String hashtag, TimeSpan timespan ) throws Exception {
		...
		for ( JCas jcas : jCases ) {
			...
			while ( senseIterator.hasNext() ) {
				...
				if ( !headers.contains( sense.getSenseValue() ) ) {
					...
					subClusters.add( createClusterElements( jCases, sense.getSenseValue(), headers ) );
				}
			}
		}
