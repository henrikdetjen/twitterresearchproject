...
public class HashtagStreamWriter {
	...
	private String path;
	/**
	 * interval after that a new json-file will be created, where the stream is written in
	 * in milliseconds
	 */
	...
	private void checkTimeout() {
		...
		if( now.after(nextTimeout) ){
			...
			//timeout -> change file
			file = new File(path + "/" + TimeStamp.createLong(new Date(nextTimeout.getTime() - interval)) + "_" + this.hashtag +".json");
